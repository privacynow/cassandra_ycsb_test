...
then
  ...
  # Launch server nodes
  instance_id_array=( $(ec2-run-instances $ami_id -region $region -k $EC2_KEY_NAME -n $server_count -z $availability -t $server_instance_type -a ":0:$subnet_id:::$security_group_id" --placement-group $placement_group --associate-public-ip-address true | grep "^INSTANCE" | cut -f 2) )
  ...
  do
    ...
    echo ${instance_id_array[i]} >> $server_instance_file
  done
else
